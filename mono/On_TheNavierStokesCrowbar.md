...
**Postscript, September 2026.** OpenAI announced (September 8, 2026) a 166-page proof, with Lean formalization in progress, of finite-time blowup for 3D incompressible Navier–Stokes under smooth external forcing — a resolution of Clay statements C and D. The construction is free-space, viscous, and produces a narrowing vortex whose radial contraction runs faster than its axial elongation, with peak velocity diverging while total kinetic energy stays bounded. Independent verification is not yet established; a priority dispute involving Buckmaster (NYU) and Alpöge (Anthropic) is in progress; Palasek–Tao have flagged specific obstacles to removing the forcing, so Clay statements A and B (the unforced case) remain genuinely open. The construction's shape is a direct instantiation of the cascade the corpus subsequently composed in `L4-TheSpiralingCascade` §VI — infinite scale-refinement completing in finite total time via convergent geometric-series summation, with the physical singularity avoided in real fluids only by the substrate floor at $\lambda_{\text{mfp}}$ (that file's §VII). What OpenAI's announcement does to this file's position: it further narrows the residual free-space viscous question to the unforced case, and it confirms — on the forced side — that the framework's "molecular dissolution at $\lambda_{\text{mfp}}$" observation is doing the right structural work. It does not resurrect any of the file's dead claims.
