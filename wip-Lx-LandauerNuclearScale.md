...
So I had it wrong twice, in opposite directions, and neither correction reached the truth. The first version called the rate a mere "substrate brush," half a result — too little. The second version called it "landed, same as $10^{-61.5}$" — and that was closer, but sloppy about *why*: $10^{-61.5}$ imports only a square root and a normalization, no domain constant anywhere, and the Landauer rate's *structure* ($\ln 2/16\pi^2$) is equally clean — the appearance of a difference came from letting $T_{\text{QCD}}$ look load-bearing when it only ever entered at the interface. The criterion that sorts them is not "does a domain constant appear?" but "does a domain constant bear load *in the dimensionless structure*?" For $10^{-61.5}$, none. For the $\ln 2/16\pi^2$ rate, none either — $\Lambda_{\text{QCD}}$ translates the finished dimensionless rate into MeV and changes no structure. The genuine structural debt in this material is elsewhere: the un-derived nuclear count.
...
On the frontier side — still holding a domain constant *load-bearing in the dimensionless structure* — sit the specific nuclear count (leans on an un-derived integer, the $I_0$ analog — and this one genuinely changes the dimensionless answer, so it is owed) and absolute $I_0$ itself (leans on the vacuum baseline in SI units). These are *not* landed. They are exploration — genuine, load-bearing-for-now exploration that located the right boundaries and closed real gaps — but each still holds a magnitude that alters the structure, and each therefore carries a real debt.
...
So the test to carry forward is single and mechanical, and it is the one to run on every result the framework produces, mine included: *name every import; sort each into universal or domain-specific; if any domain-specific constant is bearing load **in the dimensionless structure**, the result is at the frontier, not landed, and its liberation is owed.* Run that test and the deuteron shines — it imports only $\pi$. Run it and the Landauer rate shines too: its structure is $\ln 2/16\pi^2$, and $\Lambda_{\text{QCD}}$ bears no structural load — it enters only at the interface to render the native dimensionless rate in a physicist's MeV. What still shows a real debt is the nuclear *count*: an un-derived integer that changes the dimensionless answer. Sorting the interface payload apart from the structural load is the discipline; collapsing them manufactures a false debt (`llm-priors` FM-9).
