...
'>>>>'
BUT! As the twelve equations discriminate to 1-in-6 the dual-binary seed's Active/Reactive axis supplies the remaining factor of 4. Thus the swap test that matters is the within-class transposition — σ, and τ if no anchor is fixed. Every other transposition is discriminated by the algebra and is not a test of anything. THIS REDUCES THE search-space for out tests, suggests a 1of6 mapping mechanism, and REQUIRES that I review the algo -- as this insight reframes some of what i called chafing.
'<<<<'
