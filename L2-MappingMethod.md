...
## 0. Upstream of Sorting: The Move That Produced the Frame

This file teaches how to apply the four-fold to a domain. It has historically opened at the sorting procedure, which presumes the four-fold already in hand, and left the move that *produces* a four-fold undocumented. That omission has cost every reader who arrived wondering where the frame came from, and it has cost instances who met the frame's fingerprint in a dozen files and reported each sighting as a discovery.

The generative move is one move.

Take a closed system of three mutually-determining terms. It is complete on its own terms and it is flat — relations without volume, and no access to whatever contains it. Add a fourth term that is **properly orthogonal**: not derivable from the three, not a relabelling of a combination of them. Four consequences follow together. The three acquire a volume. The algebraic space multiplies. Knots that were open in the plane close. And the containing topology's integrations become available — you gain the operations of a volume where you had only the operations of an area.

The orthogonal +1 is a viewing position. It is the 3D view on the 2D plane, taken deliberately, as a technique for reaching the containing topology in order to use what it affords.

**Ohm and Faraday are the origin case, not an analogy for it.** $U = IR$ is closed, complete, correct, and flat. $P$ is not derivable from it: power brings energy-flow, and therefore time. Adding $P$ produces the twelve equations of `L3-FormalFoundations` §I, gives Ohm's relation a volume, doubles the algebraic space, closes the knot, and makes alternating current computable — impedance, phase angle, reactive against real power, an entire computational world downstream of one properly orthogonal term. The corpus's four-fold *is* that move, and the K3 → K4 ascent is that move stated topologically.

**Orthogonality carries the entire load.** A fourth that can be derived from the three adds nothing; you have renamed a combination and gained no volume. The test is never "can a fourth thing be named here" — a fourth thing can always be named. The test is whether the candidate is independent of the three in the way $P$ is independent of $U$, $I$, and $R$. Fail that test and what you have is decoration, and the honest word for it is decoration.

**Consequence for the sorting procedure below.** When you map a domain's four elements onto the poles, you are applying a frame that was itself generated by this move, to a domain you are implicitly claiming admits it. Where a domain presents a closed and satisfying three, the live question is not whether a fourth *exists* but whether the domain's own fourth is properly orthogonal — and if it is, what integrations its volume makes available that the flat three could not reach. That question is answerable per domain and is where the real work is. Noting that a domain has a three-plus-one shape is not a finding; it is the instrument reporting its own presence.
