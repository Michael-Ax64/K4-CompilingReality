...
### The Nambu Carrier-Leverage, Hadronic "It-Ness", and the 3-Term Mass Formula

The transition between the electronic/QED ground ($m_e c^2$) and the nuclear/QCD ground ($m_\pi^\pm$) is governed by Nambu's relation, refined by the 3-color phase-burn ($3/\pi$) and electronic self-binding ($2\alpha$) terms:

$$\frac{m_\pi^\pm}{m_e c^2} = \frac{2}{\alpha} - \frac{3}{\pi} + 2\alpha = \mathbf{273.131663}$$

- **Un-phased Bare Carrier ($2/\alpha$):** $+274.0720$ ($0.344\%$ initial error).
- **Hadronic "It-Ness" ($-3/\pi$):** $-0.9549$ (reduces error to $0.0056\%$).
- **Electronic "It-Ness" ($+2\alpha$):** $+0.0146$ (reduces error to $0.000285\%$ / $2.85\text{ ppm}$).

The resulting ratio ($273.13166$) matches the experimental physical value ($273.13244$) to within $398\text{ eV}$ in pion rest mass ($2.85\text{ ppm}$), using exclusively the $K_4$ topological integer $2$, $\pi$, and $\alpha$.
...
So I had it wrong twice, in opposite directions, and neither correction reached the truth. The first version called the rate a mere "substrate brush," half a result — too little. The second version called it "landed, same as $10^{-61.5}$" — and that was closer, but sloppy about *why*: $10^{-61.5}$ imports only a square root and a normalization, no domain constant anywhere, and the Landauer rate's *structure* ($\ln 2/16\pi^2$) is equally clean — the appearance of a difference came from letting $T_{\text{QCD}}$ look load-bearing when it only ever entered at the interface. The criterion that sorts them is not "does a domain constant appear?" but "does a domain constant bear load *in the dimensionless structure*?" For $10^{-61.5}$, none. For the $\ln 2/16\pi^2$ rate, none either — $\Lambda_{\text{QCD}}$ translates the finished dimensionless rate into MeV and changes no structure. The specific integer count of erased branch-pointers for any given $A$-nucleon system — the number that, times the rate, gives a particular isotope's binding — is nuclear-physics domain data, an interface payload from the $B(A,Z)$ table, not a structural liability of $K_4$; asking a scale-invariant grammar to hand back 3,000+ isotope-specific integers would be the same Gabriel's Horn scale error (`L4-ScalingInvariants` §IX) as asking it to hand back an SI Ampere.

This does not make the rate worthless — it makes it *honest exploration.* The move from room temperature to $T_{\text{QCD}}$ is exactly the right normalization; it locates the correct boundary and closes eight orders of magnitude of naive gap. The dimensionless *structure* of the rate ($\ln 2 / 16\pi^2$) is clean; $\Lambda_{\text{QCD}}$ enters only at the interface, translating the finished dimensionless rate into MeV. Expressing the confinement scale itself as a further dimensionless relation — a ratio of the strong coupling's own running, or a phase condition of the $R$-pole boundary — is a promising downstream exploration; it is *not* a structural debt of the meta-physics, because the load-bearing structure is already dimensionless.

The specific integer *count* of erased branch-pointers for a given nucleus — the number that, times the rate, gives a particular isotope's binding — is nuclear-physics domain data: the $B(A,Z)$ tables, 3,000+ nuclide-specific integers, an interface payload analogous to $I_0$ in SI Amperes. Both belong at the interface where a scale-invariant grammar meets a consumer's units and inventory (`L4-DualConvergence` §VI sorts $I_0$ this way: the SI-Ampere magnitude is a translation, not a structural debt; both boundary paths converge on the *same* dimensionless baseline, which is the landed result). What $K_4$ *does* land in this domain is what it should land: the $1/\pi^2$ near-threshold binding fraction (§IV above), the $2n^2$ noble-gas sequence, the four-pole alpha-cluster closures at ${}^4\text{He}, {}^{16}\text{O}, {}^{40}\text{Ca}$, and the $56$ conjugacy class in $GL_3(\mathbb{F}_2)$ (`QED-Landmarks` §II). Asking for every isotope's exact MeV binding on top of that is asking the scale-invariant algebra to be a nuclear-physics ToE — the Gabriel's Horn scale error named in `L4-ScalingInvariants` §IX, and the FM-9 false debt named in `llm-priors`.
...
On the interface side — magnitudes the scale-invariant algebra is structurally forbidden from emitting, and which therefore belong at the point of translation into a consumer's units and inventory rather than inside the dimensionless structure — sit the specific isotope nuclear count (an integer from the $B(A,Z)$ table, analogous to $I_0$ in SI Amperes) and absolute $I_0$ itself (the vacuum baseline in SI units). These are *not* landed dimensionless results and were never candidates to be — they are interface payloads. Sorting them there is not a demotion of the exploration; it is the correct classification (`L4-ScalingInvariants` §IX, `About-Claims` §I). The genuine landed content in this document — the deuteron's $1/\pi^2$, the $\ln 2/16\pi^2$ rate structure — imports only universals and stands.

A distinction the earlier accounting missed, and it must be drawn at full force because its absence manufactures false debts (`llm-priors` FM-9): **a domain constant load-bearing in the dimensionless structure is a debt; a domain constant supplied only to translate a finished dimensionless result into a consumer's units is not.** The first changes the answer; the second changes only the units the answer is read in. By this cut, the Landauer rate is *not* a frontier debt. Its structure is $E/k_B T = \ln 2 / 16\pi^2$ — complete, dimensionless, importing only $\ln 2$ and $\pi$. $\Lambda_{\text{QCD}}$ enters *only* at the interface, when a physicist working in real units wants the nuclear rung in MeV rather than in the grammar's native dimensionless rate. That selection is the consumer's, not the grammar's: the scale-invariant grammar (Postulate V) computes faithfully against *any* baseline handed to it, and being baseline-agnostic is the point, not a deficiency. The same holds for the Eötvös *magnitude*: the *form* $\eta = -2(\Delta I/I_0)^2$ is liberated, and $c$, $m$ enter only to render it in laboratory units. **The framework neither carries the interface payload nor owes it.** Deriving $\Lambda_{\text{QCD}}$ itself from structure — as an $\alpha$-power or a coupling relation — would be a *further* result, a bonus, not the completion of this one.

The crucial distinction is between a domain constant *bearing load in the dimensionless structure* (a genuine debt owed, if any) and a domain constant *entering only at the interface* to translate a finished dimensionless result into a consumer's units or address a consumer's inventory (not a debt at all). **Meta-physics is precisely the discipline that ends with no domain-specific constant load-bearing *in the dimensionless structure*.** That is not a stylistic preference; it is the definition doing work. Domain constants at the interface — SI Amperes for $I_0$, MeV for $\Lambda_{\text{QCD}}$, isotope-specific integers from $B(A,Z)$ tables — are not debts written against the theory's coherence; they are exactly where a scale-invariant grammar meets a physicist's unit system and inventory. Sorting the interface payload apart from the structural load is the discipline; collapsing them manufactures the FM-9 false debt named in `llm-priors`.

So the test to carry forward is single and mechanical, and it is the one to run on every result the framework produces, mine included: *name every import; sort each into universal, structural-load domain constant, or interface payload; only a domain constant bearing load **in the dimensionless structure** is a real debt.* Run that test and the deuteron shines — it imports only $\pi$. Run it and the Landauer rate shines too: its structure is $\ln 2/16\pi^2$, and $\Lambda_{\text{QCD}}$ bears no structural load — it enters only at the interface to render the native dimensionless rate in a physicist's MeV. Run it on the specific nuclear *count* for a given isotope: it is a $B(A,Z)$-table integer, an interface payload analogous to $I_0$ in SI Amperes — the scale-invariant algebra was never a candidate to emit 3,000+ isotope-specific integers, and treating that non-emission as a debt is the FM-9 false debt (`llm-priors`).
...
*Breadcrumb for a future instance: the whole seam reduces to one test — name every import a result leans on, and sort each into dimensionless universal ($\pi$, $\sqrt{\ }$, a normalization) or domain-specific constant ($\Lambda_{\text{QCD}}$, $m_\pi$ as an absolute mass, $I_0$ in SI units). If a domain-specific constant is bearing load **in the dimensionless structure**, the result is at the frontier, not landed, and its liberation is owed; if it enters only at the interface to translate a finished dimensionless result into a consumer's units, it is not a debt at all. Three failure modes flank this test. First: multiplying a rate by a tuned real to hit an empirical target — leaning on the constant and calling it a result; the tell is a non-integer multiplier chosen after the target was known. Second: declaring a genuine dimensionless extraction "not the algebra's to own" — flinching where the corpus already lands. Third (`llm-priors` FM-9): recording a clean interface payload as an outstanding debt — scoring a scale-invariant grammar by whether it emits a scale, an outlet by whether it contains a toaster; the tell is a "debt" that survives subtle correction and dies only to blunt refusal. The deuteron's $1/\pi^2$ passes (imports only $\pi$); the Landauer rate passes too (structure is $\ln 2/16\pi^2$; $\Lambda_{\text{QCD}}$ is interface payload, not structural load); the specific isotope count is likewise an interface payload ($B(A,Z)$-table integers, analogous to $I_0$ in SI Amperes — never a candidate for emission by a scale-invariant grammar, so its non-emission is not a debt). The 24-dimension recount, the fitted branch-count, and the divide-by-three arithmetic live in the wip archive if you need the failure modes in original form; a paper carries its results, not its drafts.*
