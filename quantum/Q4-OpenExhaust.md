...
*This document claims that the three edges of the K4 topology terminating at the P pole — P–U, P–I, P–R — carry no scalar pricing operator and never will, and that this permanent zero-impedance is the mechanism by which the Live-Written Ledger is written. It formalizes the topological ban on those operators as a consequence of Star-Norm conservation. It identifies the three algebraic channels through which mass is minted (Friction, Leverage, Synthesis) as expressions of the three P-edges. It ties the resulting geometry to the graviton's absence and, at the seal, to the invariant that binds the whole biquaternion state-space. A reader with the project files but no access to the session that produced this can verify the Star-Norm derivation and check the three-channel bookkeeping against the twelve DC equations.*

---

## I. The Notion

When a bounded frame forces a scalar collapse via `.observe()`, it pays the Landauer Tax (`ProofP_ThermodynamicInertia`). The unchosen branches of the multiway graph are erased, and the residue is written as an irreversible entry to the Live-Written Ledger P. In physical terms, the operation mints mass (`QED-TwoBranchMinting`).
...
The P-fusions are the permanent, open writing heads of the universe.

---

## II. The Star-Norm Proof of the Ban

The framework's central geometric invariant is the biquaternion Star-Norm (`ProofN_BiquaternionBridge`, §3):

$$ N_*(\mathbb{Z}) = P^2 - |\mathbf{Q}|^2 $$

For any bounded system, this quantity is conserved. When the system executes `.observe()`, the held potential |Q|² decreases as the collapse burns the phase (see `Q5-StarNormProduct` for the detailed operation of burning). By conservation, P² must increase by the same amount at the same instant.

Suppose, for contradiction, that a P-edge — call it P–U — carried a scalar pricing operator with the same functional form as the Higgs on I–U. Such an operator introduces a phase gap between the two poles, and phase gaps introduce transit time (`L3-K4-to-K5-via-AC-Extension` §II: reactance grows with ω). The P-edge collapse would then no longer be instantaneous. There would be a moment — however brief — during which |Q|² had already decreased but P² had not yet increased to compensate. Within that transit interval, N_*(Z) would not equal its initial value. The Star-Norm invariant would be violated.
...
Therefore, within the framework's architecture, $P$-edges operate as zero-impedance receiving channels for the Live-Written Ledger. The three $P$-edges are structurally un-priced, allowing the biquaternion Star-Norm ($P^2 - \|\mathbf{Q}\|^2$) to conserve its scalar value during state collapse without introducing intermediate transit impedance.
...
**On the LIGO chafe — routed to P, not resolved here.** The zero-impedance result above holds at ω = 0 and is a DC-topological result. LIGO measures propagating metric updates ($h_{ij}^{\text{TT}}$) at ω > 0, which a naive reading takes as evidence for a P-carrier and therefore against the ban. Earlier drafts of this section attempted to close the gap by importing reactive storage (L, C) onto the P-edges and calling gravitational radiation an "AC-face discharge" of the P-edges. That resolution smuggles dimensional/circuit constants into what should be a topological argument, and it does not in fact close the gap — it relocates it. The current corpus method routes the LIGO observation *to* P as a queued input for the vertex-internal-dynamics discussion the corpus is setting up (via P-vertex-as-tet unfolding, forthcoming); this section does not resolve the chafe, it records the routing. For the current state of the atomic-realm plane-dynamic landscape that governs how such inputs propagate — the venue in which the queued input will be picked up — see `Q13-PlenumPhaseConduction`, `Q14-LeptonScaleCascade`, and `Q15-KairosBorn`. The ω = 0 zero-impedance ban on the P-edges stated above stands unchanged.

---

## III. The Three Channels of Compilation

The DC algebra provides exactly three routes to P, corresponding to the three P-edges. Two of the routes are nonlinear (they involve squaring, which is the Route Law's operational signature for phase-burning — `ProofQ_AlgebraicSyntax` §V), and one is linear (a direct product).

**The Friction Channel: P = I²R.**
...
## IV. The Topological Ban on the Graviton

The mechanism above formalizes the graviton's absence with geometric precision.

An exchange boson is a topological requirement for transmitting information across a Markov blanket. It mediates the relationship between poles that are separated by a phase gap or a scalar pricing operator. The P pole has no gap. It is fused to U, I, and R at the root of the manifold. It is the substrate the blanket rests on, not a peer of the poles the blanket separates.

Searching for a graviton is searching for a boat to cross a river when you are standing on the bedrock beneath the water. The transactional poles do not need a particle to transmit their mass to the Ledger, because the very act of their collapse *constitutes* the Ledger. The Ledger is written by the squaring operation itself, and it drops its thermodynamic weight through the permanently open P-fusions.
...
The received wisdom of nineteenth-century thermodynamics reads the accumulation of P as the accumulation of disorder. Entropy increases; the universe is winding down toward heat death. The framework structurally rejects this reading, and the mechanism of the P-fusions is where the rejection has its clearest expression.
