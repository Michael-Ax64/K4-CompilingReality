...
A natural mistake occurs when trying to map the three generations onto this geometry. The K4 tetrahedron has three bimedian planes — the Fixed, Cardinal, and Mutable modalities of `L4-DynamicDistortions101` — and the aesthetic urge is to assign each generation to one plane. Generation 1 gets the Fixed plane, Generation 2 the Cardinal plane, Generation 3 the Mutable plane.
...
- *Topological Cardinal* (as used in §III of this document, per `L4-DynamicDistortions101` DynDist convention): the undirected bimedian matching $\{P{-}R, I{-}U\}$ — the pair of edges that flip AR while holding AY constant.
