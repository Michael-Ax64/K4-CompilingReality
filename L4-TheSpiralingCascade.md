...
**The stage.** Each level of vortex refinement is a $K_4$ volume executing a bimedian shear. The plane doing the shearing is the one orthogonal to the vortex axis: radial contraction is the bimedian area $A(\theta_n) = A_0 \cos\theta_n$ collapsing as $\theta_n \to \pi/2$. Axial elongation is the compensatory phase-transfer onto the perpendicular plane. The shear commits its Landauer Tax as viscous plus anomalous dissipation (per the compound ledger $P_{\text{total}}$ of `On_TheNavierStokesCrowbar` §III.3); the committed state becomes the boundary condition for the inner core.

**The sequence.** Successive vortex cores at $r_n = r_0 \lambda^{-n}$ carry increasing angular velocity $\omega_n = \omega_0 \lambda^n$ by conservation of angular momentum — the classical spinning-figure-skater's-arms picture, running self-similarly across scales. The local circulation timescale $\Delta t_n \sim 1/\omega_n$ shrinks geometrically.

**The sum.** $T^{\ast} = \Delta t_0 / (1 - \lambda^{-1}) < \infty$. Infinitely many increasingly rapid inner rotations complete in finite coordinate time. The peak vorticity, which grows as $\omega_n$, diverges at $T^{\ast}$.

**The finite-energy condition.** Kinetic energy density is $|\mathbf{u}|^2$ integrated over volume. If radial contraction ($r_n^{-2}$ per unit axial length in 2D cross-section) runs faster than the axial elongation is stretching mass into the tube, the total volume of the divergent-velocity region shrinks fast enough to keep $\int |\mathbf{u}|^2 \, dx$ bounded. This is the anisotropy of §III, which distinguishes the admissible cascade from the exactly-self-similar case NRŠ ruled out. In $K_4$ terms: the shear collapses one bimedian faster than the phase-transfer to the other bimedians recovers the volume — the total interior contracts even as the local intensity diverges.

**What the corpus reads that fluid textbooks do not.** The spaghetti vortex is not "an exotic pathology of the mathematical idealization." It is what any resonator with an unbounded driver, a scale-invariant operation-shape, and anisotropic shearing does. The unbounded driver here is the smooth external forcing (Clay statement D, not statement B — the unforced case remains genuinely open per Palasek–Tao's flagged obstacles). The scale-invariance is Navier–Stokes' own $\mathbf{u}(x,t) \to \lambda \mathbf{u}(\lambda x, \lambda^2 t)$ symmetry, which is the fluid instantiation of Postulate V. The anisotropy is the specific geometric fact that vortex-stretching in 3D can concentrate vorticity while diluting the volume it occupies — a fact carried by the parity-even eigenvalue geometry of the trace-free strain tensor (`On_TheNavierStokesCrowbar` §II.2), not by handedness.
